...
main:
    ...
    #solve puzzle and get enough water here
    #request puzzle to a certain address
    #*REQUEST_PUZZLE = &board;
    
    
	

    #part 1 code, it'll work after getting enough water
    #be very conservative with the water

    #write sections for left vs right side later


	lw $t0, BOT_X
    ble $t0, 200, starting_on_left
    j starting_on_right

starting_on_right:

jal solve_single_puzzle
    li $a0, 0x000400fe
    jal wash
    ...
    #get more water HERE, 1 puzzles at a time
...
begin_while_a:
    lw $t1, has_puzzle
    bne $t1, 0, end_while_a
    j begin_while_a
end_while_a:
    #puzzle has loaded, solve the puzzle
    la $a0, board
    jal quant_solve
    la $t0, board
...
wait_cycles:
    #$a0 will be how many cycles to wait
    lw $t2, TIMER #t0 is begin timer
wait_cycles_loop:
    lw $t3, TIMER
    sub $t3, $t3, $t2
    bge $t3, $a0, end_wait_cycles
    j wait_cycles_loop
end_wait_cycles:
    jr $ra
...
#issues: if theres ever an interrput, there a chance that t1, t2, t3 etc get used. also, a0 gets used by interrupt dispatch
move_left:
	li $t1, 180
	sw $t1, ANGLE
	li $t1, 1
...
loop_move_up:
	...
	li $t4, 10
	sw $t4, VELOCITY
	bge $t2, $t3, loop_move_up
	#only jump back after x has changed by 1
	li $t1, 0
	sw $t1, VELOCITY
	jr $ra
move_down:
